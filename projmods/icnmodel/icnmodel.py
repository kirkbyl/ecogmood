...
class LinModelICN(object):
    ...
    def getFeatures(self, metric='IMS', calcVar=True, deltaT=-600, interval=1200, window=60, plotCorr=False, band=None, shuffle=False, **kwargs):
        ...
        if band is not None:
            ...
            self.band = raw_input("Enter band of interest. Choose from {0} or 'all': ".format(str(self.allBands)))
        ...
        featureMatrix, score = linmod.extract_ICN_features(self.patient, self.ICNprojs, self.ICNtaxis, metric=metric, deltaT=deltaT, interval=interval, window=window, calcVar=calcVar, plotCorr=plotCorr, shuffle=shuffle)
    ...
    def plotCoeffs(self):
        ...
        plotcoherence.plot_matrix_single(self.ICNs[:,:,bestICN], dataType='ICA', fig=fig, ax=ax[1])
        ax[0].set_title('ICN coeffs for patient {} in {} band'.format(self.patientID, self.band), fontsize=12)
    ...
    def getTestFeatures(self, calcVar=None, window=None):
        ...
        if calcVar == True:
            allFeatures, allFeatures_tAxis = IMSmodule.matrix_variance(self.ICNprojs, self.ICNtaxis, window)
        elif calcVar == False:
