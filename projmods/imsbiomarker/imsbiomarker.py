...
class PooledPatients(object):
    ...
    def loadAll(self, band, regs, dtype='coherence', tBin=10, matType='average'):
        ...
        for pID in self.patientIDs:
            ...
            if regs not in self.data[datatype][pID]:
                print('Loading {0} {1} data for {2}'.format(regs, datatype, pID))
                self.data[datatype][pID][regs], self.data[datatype][pID]['tAxis'] = load_data(self.patients[pID], dtype, band, regs, tBin=10, matType='average')       
               
            else:
                print('Already have {0} {1} data for {2}'.format(regs, datatype, pID))
    ...
    def calcVarAll(self, band, regs, dtype='coherence', window=60):
        ...
        for pID in self.patientIDs:
            ...
            print('Calculating variance for {0} {1} data for {2}'.format(regs, datatype, pID))
            if dtype == 'coherence':
                self.dataVar[datatype][pID][regs], self.dataVar[datatype][pID]['tAxis'] = IMSmodule.calculate_coh_variance(self.data[datatype][pID][regs], self.data[datatype][pID]['tAxis'], window=window)       
            elif dtype == 'power':
                self.dataVar[datatype][pID][regs], self.dataVar[datatype][pID]['tAxis'] = IMSmodule.calculate_power_variance(self.data[datatype][pID][regs], self.data[datatype][pID]['tAxis'], window=window)
    ...
    def getIMSfeatures(self, band, regs, dtype='coherence', tBin=10, matType='average', calcVar=True, deltaT=-600, interval=1200, window=60, plotCorr=True):
        ...
        for pID in self.patientIDs:
            ...
            if feature is not None:                                      
                ...
                IMStimes = [time.mktime(tt.timetuple()) for tt in self.patients[pID].IMS['IMStimes']]
                        
                IMSdataframe = IMSmodule.IMS_vs_feature(IMSpts, IMStimes, feature, featuretAxis, deltaT=deltaT, interval=interval)
                ...
                ylabel = 'IMS'
                title = 'Patient: {0}, Band: {1}, Regions: {2}'.format(pID, band, '-'.join(regList))  
    
                self.figLabels['title'] = 'Regions: {0}, Band: {1}'.format('-'.join(regList), band)
    ...
    def plotPooled(self, band, regs, normType='z-score', showPlot=True, linearFit=True, plotRange=False, pctileRange=[25, 75], dtype='coherence', tBin=10, matType='average', calcVar=True, deltaT=-600, interval=1200, window=60, plotCorr=False, legendLoc=0):
        ...
        """
        self.getIMSfeatures(band, regs, dtype=dtype, tBin=tBin, matType=matType, calcVar=calcVar, deltaT=deltaT, interval=interval, window=window, plotCorr=plotCorr)
        ...
        featureRange = self.getFeatureRange(featureDict, regs, percentile=pctileRange)
        ...
        for pID, ims, feat in zip(self.patientIDs, self.IMS_all, self.feature_all):
            ...
            if normType == 'z-score':
                ...
                i_norm = [int(round(((float(II)-IMSrange[0])/(IMSrange[1]-IMSrange[0]))*100)) for II in ims]
                f_norm = feat
        ...
        if showPlot == True:
            ...
            for vv, ii, cc, mm, ss, pp, ff in zip(feature_norm, IMS_norm, colors, markers, sizes, self.patientIDs, featureRange):
                plt.scatter(vv, ii, marker=mm, s=ss, c=rgb2hex(cc), edgecolor=rgb2hex(cc), label=pp)
                if normType == 'normIMS' and plotRange == True:
                    plt.errorbar(np.median(vv), np.mean(ii), xerr=[[np.median(vv)-ff[0]], [ff[-1]]], yerr=[[np.std(ii)], [np.std(ii)]], fmt='k--o', ecolor='k')
            if linearFit == True:
                plotcorrelations.scatter_regplot(np.array(feature_join), np.array(IMS_join), ax=ax, scatter_kws={'s': 0}, color='k')
    ...
    def plotHeatmap(self, band, regs, deltaTs=[-1800, -600, -300, 0, 300, 600], intervals=[10, 60, 300, 600, 1200, 1800], dtype='coherence', tBin=10, matType='average', calcVar=True, window=60, showCorrPlot=False, clim=[-12, 0]):
        ...
        for dd, deltaT in enumerate(deltaTs):
            ...
            for ii, interval in enumerate(intervals):
                ...
                plotOut = self.plotPooled(band, regs, deltaT=deltaT, interval=interval, plotCorr=False, dtype=dtype, tBin=tBin, matType=matType, calcVar=calcVar, window=window, showPlot=showCorrPlot)
                
                self.pMatrix[dd, ii] = np.log10(plotOut['p'])
                self.rMatrix[dd, ii] = plotOut['r2']
                
        fig, ax = plt.subplots(figsize=(8,7))
        plt.imshow(self.pMatrix, cmap='bone', interpolation='None', aspect='auto', clim=clim)
        plt.colorbar()       
    ...
    def plotTraits(self, band, regs, pctileRange=[25, 50, 75], dtype='coherence', tBin=10, matType='average', calcVar=True, window=60):
        ...
        featureRange = np.array(self.getFeatureRange(featureDict, regs, percentile=pctileRange))
        ...
        for trait in ['BDI', 'BAI', 'PHQ9', 'GAD7', 'Rum']:
            ...
            fig, ax = plt.subplots()
            plt.errorbar(traitAll, featureRange[:,1], yerr=[featureRange[:,1]-featureRange[:,0], featureRange[:,-1]], fmt='ko', ecolor='k')
            plt.title(trait)
...
def find_feature(patient, dtype, band, regs, tBin=10, matType='average', calcVar=True, window=60):
    ...
    try:
        if dtype == 'coherence':
            ...
            if calcVar == True:
                feature_all, featuretAxis = IMSmodule.calculate_coh_variance(cohOut['coherence'], cohOut['tAxis']['UTC'], window=window)
            else:
            ...
            if calcVar == True:
                feature_all, featuretAxis = IMSmodule.calculate_power_variance(powOut['power'], powOut['tAxis']['UTC'], window=window)
            else:
