...
class IntrinsicNetwork(object):
    ...
    def __init__(self, pID, band, dataPath=None, commonRegbins='default_subset', method='ICA'):
        ...
        if band == 'all':
            for bb, bnd in enumerate(self.allBands):
                ...
                icnDF = loaddata.icn_dataframe(
                    self.patient.ID, self.patient.ICNpath)
    ...
    def rerunICN(self, **kwargs):
        ...
        icnProj = cohmodule.project_onto_component(
            self.coh, icnMat, symmMat=True)
    ...
    def runPCA(self, nPCs=50, matType='half', meanSubtract=True, plotPCA=True, subset=range(10), saveFig=False, figname='PCA', saveDir='default'):
        ...
        PCs, eigVs = cohmodule.pca_calculation(
            self.coh, nPCs=nPCs, pcaType=matType, meanSubtract=meanSubtract)

        if plotPCA == True:
            fig = plotcoherence.plot_matrices(
                PCs, dataType='PCA', subset=subset)
    ...
    def getCommonICNs(self, regBins=None, nBinSplit=4):
        ...
        *** refer to ecogtools.recordingparams.elecs for abbreviation definitions ***)
        ...
        self.commonICN = np.zeros(
            (len(regBins)*nBinSplit, len(regBins)*nBinSplit, self.ICN.shape[-1]))

        regPairs = utils.get_pairs(
            regBins, regBins, includeSame=True, unique=False)
        ...
        for rp, regPair in enumerate(regPairs):
            ...
            if elecs1 is not None and elecs2 is not None:
                ...
                for ii, ee in zip(inds1, elecs1[0:nBinSplit]):
                    self.commonICN[ii, inds2, :] = self.ICN[ee,
                                                            elecs2[0:nBinSplit], :]
    ...
    def plotICN(self, subset='default', saveFig=False, figname='ICN', saveDir='default', plotNew=False, plotAve=True, plotCommon=True, **kwargs):
        ...
        fig = plotcoherence.plot_matrices(
            plotICN, dataType='ICA', subset=subset, title=title, **kwargs)
        if plotAve == True:
            if not hasattr(self, 'aveICN'):
                self.getAveICNs()
            plotcoherence.plot_matrices(
                self.aveICN, dataType='ICA', subset=subset, title=title+', ave ICNs', **kwargs)
        if plotCommon == True:
            if not hasattr(self, 'commonICN'):
                self.getCommonICNs()
            plotcoherence.plot_matrices(
                self.commonICN, dataType='ICA', subset=subset, title=title+', common ICNs', **kwargs)
    ...
    def plotCoh(self, subset='default', saveFig=False, figname='Coh', saveDir='default', **kwargs):
        ...
        fig = plotcoherence.plot_matrices(
            self.coh, dataType='Coherence', subset=subset)
    ...
    def saveFig(self, fig, figname='default', saveDir='default', figtype='eps', **kwargs):
        ...
        utils.saveFig(fig, figname=figname, saveDir=saveDir,
                      figtype=figtype, **kwargs)
...
class IcnGraph(object):
    ...
    def __init__(self, pID, band, dataPath=None, method='ICA', graphType='common', quantileThresh='default', weightThresh='default'):
        ...
        self.constructGraphs(quantileThresh=quantileThresh,
                             weightThresh=weightThresh)
    ...
    def plotGraphs(self, subset='default', radius=10, noderadius='default', alpha='default', linewidth='default', plotSeparate=False, fig=None, ax=None, colormap=cm.plasma):
        ...
        if plotSeparate == False:
            fig = plotchord.plot_graphs(self.graphs, subset=subset, title=title, nodeLabels=nodeLabels, radius=radius, noderadius=noderadius,
                                        alpha=alpha, nodecolor=nodecolor, nodefill=nodefill, linewidth=linewidth, fig=fig, ax=ax, colormap=colormap)
        elif plotSeparate == True:
            for ss in subset:
                fig = plotchord.plot_graphs(self.graphs, subset=[ss], title=title, nodeLabels=nodeLabels, radius=radius, noderadius=noderadius,
                                            alpha=alpha, nodecolor=nodecolor, nodefill=nodefill, linewidth=linewidth, fig=fig, ax=ax, colormap=colormap)
        plt.show()
    ...
    def getEdges(self, quantile=98):
        ...
        for kk, icn in enumerate(np.rollaxis(self.graphICN, 2)):
            ...
            for tt in (thresh, -thresh):
                ...
                if tt > 0:
                    ee = [ii for ii in zip(np.where(icnSigned > tt)[
                                           0], np.where(icnSigned > tt)[1])]
    ...
    def plotBinerized(self, quantile=98, subset='default'):
        ...
        title = 'Binerized ICNs for patient {0}, {1} band'.format(
            self.patientID, self.band)
        plotcoherence.plot_matrices(
            self.icnBin, subset=subset, dataType='ICA', title=title)
    ...
    def getWeightedEdges(self, weightThresh=0.3):
        ...
        edgePairs = utils.get_pairs(range(len(nodes)), range(
            len(nodes)), includeSame=False, unique=True)
...
def common_reg_bins(regbins='default_all'):
    ...
    if regbins == 'default_all':
        regbinlist = ['aSTC', 'mSTC', 'pSTC', 'AM', 'vHPC', 'mHPC',
                      'dHPC', 'aOFC', 'pOFC', 'iCIN', 'sCIN', 'dCIN', 'INS']
    elif regbins == 'default_subset':
        regbinlist = ['aSTC', 'mSTC', 'pSTC', 'AM',
                      'vHPC', 'aOFC', 'pOFC', 'iCIN', 'sCIN', 'INS']
    else:
        regbinlist = regbins
