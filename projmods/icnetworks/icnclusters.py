...
class IcnCluster(object):
    ...
    def __init__(self, pIDs, dataPath=None, graphType='common', bands=['theta', 'alpha', 'beta', 'gamma'], quantileThresh='default', weightThresh='default'):
        ...
        self.collectGraphs(quantileThresh=quantileThresh,
                           weightThresh=weightThresh)
        self.concatICNs()
    ...
    def collectNetworks(self):
        ...
        for pp, (pID, patient) in enumerate(self.patients.items()):
            ...
            for band in self.bands:
                Network = icnetworks.IntrinsicNetwork(
                    patient, band, dataPath=self.dataPath)
    ...
    def collectGraphs(self, quantileThresh=98, weightThresh=0.3):
        ...
        for pp, (pID, patient) in enumerate(self.patients.items()):
            ...
            for band in self.bands:
                Graph = icnetworks.IcnGraph(
                    patient, band, graphType=self.graphType, quantileThresh=quantileThresh, weightThresh=weightThresh)
            ...
            for bb, band in enumerate(self.ICNetworks[pID].keys()):
                ...
                if self.graphType == 'common':
                    ...
                    if pp == 0 and bb == 0:
                        ...
                        self.allICNs_2D['all'] = np.vstack(
                            (self.allICNs_2D['all'], flatICN))
                        self.allICNs_3D['all'] = np.dstack(
                            (self.allICNs_3D['all'], icn))
                ...
                if bb == 0:
                    ...
                    self.allICNs_2D[pID] = np.vstack(
                        (self.allICNs_2D[pID], flatICN))
                    self.allICNs_3D[pID] = np.dstack(
                        (self.allICNs_3D[pID], icn))
        ...
        for key in self.aCC.keys():
            ...
            if key == 'all':
                ...
                for pp, plotMat in enumerate([self.aCC[key], sorted_aCC]):
                    ...
                    plt.colorbar()
                plt.suptitle('{0} combined: {1} clusters ({2} ICNs total)'.format(
                    key.capitalize(), str(nClusters), len(self.clusters[key])))
                plt.show()
    ...
    def findClusters(self, eps=0.5):
        ...
        for key in self.aCC.keys():
            self.clusters[key] = DBSCAN(
                eps=eps, min_samples=1).fit_predict(self.aCC[key])
        ...
        for key in self.allICNs_3D.keys():
            ...
            for uu, cc in zip(unique, counts):
                if cc >= 2:
                    ...
                    title = 'Patient: {0}, cluster {1}: {2}'.format(
                        key, uu, np.array(self.icnLabels[key])[subset])

                    if showMatrix == True:
                        plotcoherence.plot_matrices(
                            self.allICNs_3D[key], dataType='ICA', subset=subset, title=title)

                    if showGraph == True:
                        plotchord.plot_graphs(
                            self.allGraphs[key], subset=subset, title=title, nodeLabels=self.nodeLabels[key])
...
class TopographicalMap(object):
    ...
    def getSubgraphs(self, nThresh=1):
        ...
        for i in range(n):
            if len(self.subgraphs[i].nodes()) > nThresh:
                print '\tNodes in subgraph {0}: {1}'.format(
                    i, self.subgraphs[i].nodes())
                ...
                for vv, vals in enumerate(values):
                    ...
                    for val in vals:
                        ...
                        if val in Gcore.nodes():
                            ...
                            self.Gviz.add_node(
                                val, fillcolor=hexcolors[vv], shape=shape, xlabel=label, width=width, style=style, penwidth=penwidth, fontsize=9)
    ...
    def plotCliqueICNs(self, G='default', ncore=6, nclique=4, plotAll=False, ave='mean', pc=25, colormap=cm.plasma):
        ...
        if plotAll == True:
            for cc, clique in enumerate(cliques):
                plotchord.plot_graphs(self.Networks.allGraphs['all'], subset=list(
                    clique), nodeLabels=self.Networks.nodeLabels['all'], radius=10, noderadius=0.1, alpha=0.5, colormap=colormap)
        ...
        fig = plotchord.plot_graphs(
            cliqueGrphs, nodeLabels=self.Networks.nodeLabels['all'], radius=10, noderadius=0.1, alpha=0.3, linewidth=1.25, plotSeparate=False, colormap=colormap)
