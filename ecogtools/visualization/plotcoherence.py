...
def plot_matrices(data, tAxis=None, subset=range(10), dataType='Coherence', tStart=(), labels=None, title='', **kwargs):
    ...
    if tAxis is not None:
        ...
        if tStart != ():
            tStartDT = utils.convert_timestamp(tStart, isTuple=True)['datetime']
    ...
    for kk, X in enumerate(subset):
        ...
        if dataType == 'PCA':
            ...
            try:
                ax1D[kk].set_title(tAxisStr[X].strftime('%Y-%m-%d, %H:%M:%S'), fontsize=11)
            except:
                ax1D[kk].set_title('')
        plot_matrix_single(data[:,:,X], dataType, fig=fig, ax=ax1D[kk], **kwargs)
...
def plot_matrix_single(data, dataType=None, ax=None, fig=None, **kwargs):
    ...
    if ax is not None and fig is not None:
        im = ax.imshow(data, clim=clim, cmap=cmap, interpolation='none', aspect='auto', **kwargs);
        fig.colorbar(im, ax=ax)
    else:
        plt.imshow(data, clim=clim, cmap=cmap, interpolation='none', aspect='auto', **kwargs); 
        plt.colorbar()                                           
