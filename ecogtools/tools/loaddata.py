...
def coherence_dataframe(patientID, cohDir):
    ...
    for npyFile in npyFiles:
        ...
        dfRow.append({'patientID': pID, 'fqBand': band, 'tBin': timebin, 'type': matrixtype, 'filePath': npyFile})
...
def signals_dataframe(patientID, signalsDir):
    ...
    for ss, signalFile in enumerate(signalFiles):
        ...
        dfRow.append({'filename': filename, 'startT': startT, 'endT': endT, 'sampFq': sampFq, 'signalsFile': signalFile})
...
def reload_signals_from_timestamp(patientSigDF, timeStamp=None, deltaIdx=0):
    ...
    if timeStamp is None:
        ...
        sigIdx_temp = np.where((patientSigDF['startT'] <= timepoint) & (patientSigDF['endT'] >= timepoint))[0]
        if len(sigIdx_temp) == 0:
            print('No data at time {0}'.format(timepoint))
            loadIdx = None
        else:
            sigIdx = np.where((patientSigDF['startT'] <= timepoint) & (patientSigDF['endT'] >= timepoint))[0][0]
    ...
    if loadIdx is not None and loadIdx <= len(patientSigDF['signalsFile']):
        ...
        tAxis['UTC'] = np.load(patientSigDF['signalsFile'][loadIdx].replace('_signal.npy', '_time.npy'), mmap_mode = 'r')     
...
def reload_coherence(patientCohDF, band, tBin, matType='average'):
    ...
    cohOut, fileID = reload_matrix_from_dataframe(patientCohDF, band, tBin, dataName='coherence', matType=matType)
...
def reload_power(patientPowerDF, band, tBin, matType='average'):
    ...
    powerOut, fileID = reload_matrix_from_dataframe(patientPowerDF, band, tBin, dataName='power', matType=matType)
...
def reload_matrix_from_dataframe(patientDF, band, tBin, dataName='coherence', matType='average'):
    ...
    idx = np.where((patientDF['fqBand'] == band) & (patientDF['tBin'] == str(tBin)+'s') & (patientDF['type'] == ctype))[0][0]
...
def icn_dataframe(patientID, icnDir):
    ...
    for folderID in folderIDs:
        allfiles = sorted(glob.glob(dataDir+'/'+folderID+'/*'+folderID+'*.npy'))
        
        for ii, icnfile in enumerate(allfiles):
            
            band = icnfile.split('_')[-1].split('.')[0]
            
            dfRow.append({'dataType': folderID, 'fqBand': band, 'filePath': icnfile})
...
def reload_icn(patientIcnDF, band):
    ...
    for folderID in folderIDs:
        icnIdx = np.where((patientIcnDF['fqBand'] == band) & (patientIcnDF['dataType'] == folderID))[0][0]
    ...
    timeIdx = np.where((patientIcnDF['fqBand'] == 'time') & (patientIcnDF['dataType'] == 'proj'))[0][0]
