...
def read_hdf5_file(filepath):
    ...
    if fileobj != 'NaN':        
        ...
        if 'timestamp vector' in fileobj:
            ...
            if nTimePoints > 1:
                sampFq = round((nTimePoints)/(timeAxis['UTC'][-1]-timeAxis['UTC'][0]))
        ...
        if 'ECoG Array' in fileobj:
            ...
            if not isinstance(sampFq, str):
                ...
                if sampFq <= fqCut:
                    ...
                    iSplit = range(0, nPts, int(np.ceil(nPts/float(sampFq/1000.))))
                    iSplit.append(nPts)
                    
                    print('High sampFq, extracting data in chunks')
                    for ii, ind in enumerate(iSplit[:-1]):
                        #print('\t'+str(ii+1)+' out of '+str(len(iSplit)-1))
                        signalArray_temp[ind:iSplit[ii+1], :] = fileobj['ECoG Array'][ind:iSplit[ii+1], :]
            else:
    ...
    for column in h5object[key]:
        ...
        for row_number in range(len(column)):
            ...
            if isinstance(firstElement, np.uint16):
                row_data.append(''.join(map(unichr, h5object[column[row_number]][:])))
