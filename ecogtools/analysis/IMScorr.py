...
def calculate_coh_variance(cohMatrix, tAxis, window=60, center=True):
    ...
    if (nElecs_reg1 > 1) or (nElecs_reg2 > 1):
        ...
        for pair in ijPairs:
            ...
            try:
                cohvar = pd.Series(cohMatrix[pair[0], pair[1]]).rolling(window=int(intv), center=center).var()  
            except: 
                cohvar = pd.rolling_var(cohMatrix[pair[0], pair[1]], int(intv), center=center)
        ...
        try:
            cohvar = pd.Series(cohMatrix).rolling(window=int(intv), center=center).var()
        except:
...
def score_vs_feature(score, scoreTimes, feature, featuretAxis, deltaT=0, interval=600, displaytOut=False):  
    ...
    if displaytOut == True:
        if deltaT < 0:
            print('Averaging '+str(abs(deltaT))+'s before score over '+str(interval)+'s time window.')
        elif deltaT >= 0:
            print('Averaging '+str(abs(deltaT))+'s after score over '+str(interval)+'s time window.')
    ...
    for nn, tPair in enumerate(tInt):
        ...
        tInds = np.where((featuretAxis >= tPair[0]) & (featuretAxis <= tPair[1]))[0]
        
        if len(tInds) > 0:
            meanVal = float(np.mean(feature[tInds]))
            stdVal = float(np.std(feature[tInds]))
            scoreList.append({'score': score[nn], 'meanVal': meanVal, 'stdVal': stdVal})
        #else:
...
def IMS_vs_feature(IMS, IMStimes, feature, featuretAxis, deltaT=0, interval=600, displaytOut=False):  
    ...
    if displaytOut == True:
        if deltaT < 0:
            print('Averaging '+str(abs(deltaT))+'s before IMS over '+str(interval)+'s time window.')
        elif deltaT >= 0:
            print('Averaging '+str(abs(deltaT))+'s after IMS over '+str(interval)+'s time window.')
    ...
    for nn, tPair in enumerate(tInt):
        ...
        if len(tInds) > 0:
            meanVal = float(np.mean(feature[tInds]))
            stdVal = float(np.std(feature[tInds]))
            IMSlist.append({'IMS': IMS[nn], 'meanVal': meanVal, 'stdVal': stdVal})
        #else:
        #    print 'No data around IMS point '+str(nn)
...
def matrix_variance(matIn, tAxis, window=60):
    ...
    for rr, row in enumerate(matIn):
        varMat[rr] = pd.Series(row).rolling(window=int(intv), center=True).var()    
