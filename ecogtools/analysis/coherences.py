...
def calculate_coherence(signalArray, sampFq, tFrame=10, tSlide='None', tAxis='default', fqBands='default', PhaseRan=True, nRand=1):
    ...
    if tAxis == 'default':
        ...
        tMax = tDuration-tFrame+tSlide
        tAxis_split = np.arange(tAxis[0], (tAxis[-1]-tFrame+tSlide), tSlide)[:-1]
        tAxis_split_inds = np.arange(0, tMax, tSlide)[:-1]
    nTimepoints = len(tAxis_split)
    
    print('Recording duration: %0.0f sec (%0.2f mins)' % (tDuration, (tDuration/60.0)))
    ...
    for nn, tt in enumerate(tAxis_split_inds[:-1]):
        ...
        if len(ttInds) > int(sampFq):
            if (ttInds[-1]+1) <= len(tAxis):
                signalChunk = signalArray[0:nElectrodes, ttInds[0]:(ttInds[-1]+1)]
            else:
                signalChunk = signalArray[0:nElectrodes, ttInds[0]:]
        
            signalChunk = np.float32(signalChunk)
            
            Cxy, phase, fqs = mlab.cohere_pairs(signalChunk.T, ijPairs, Fs=sampFq, NFFT=int(sampFq/2))
            ...
            if PhaseRan == True:                
                ...
                for X in bands:
                    Coherence_surr[X] = np.zeros((nElectrodes, nElectrodes, nTimepoints))
                    
                # Perform randomization several times (added: 25 July 2017)
                Cxy_surr_all = np.zeros((np.array(Cxy.values()).shape[0], np.array(Cxy.values()).shape[1], nRand))
                
                for nr in range(nRand):
                    signalChunk_surr = phaseran(signalChunk.T)
                    Cxy_surr, P0, F = mlab.cohere_pairs(signalChunk_surr.T, ijPairs, Fs=sampFq, NFFT=int(sampFq/2))
                    Cxy_surr_all[:,:,nr] = np.array(Cxy_surr.values())
                
                Cxy_surr_mean = np.mean(Cxy_surr_all, axis=-1)
                ijCxyValues = np.array(np.squeeze(Cxy.values())) - Cxy_surr_mean
            ...
            for X in bands:
                Indices[X] = list(np.where((fqs >= fqBands[X][0]) & (fqs <= fqBands[X][1]))[0])
    
            if len(np.shape(np.squeeze(ijCxyValues))) == 2:
                
                MeansCoh = {}; MeansPhase = {}
                for X in bands:
                    MeansCoh[X] = np.mean(np.squeeze(ijCxyValues)[:,Indices[X]], axis=1)
                    MeansPhase[X] = np.mean(np.squeeze(ijphaseValues)[:,Indices[X]], axis=1)
                ...
                for X in bands:
                    ...
                    MeansPhase[X] = np.mean(ijphaseValues[:,Indices[X]], axis=2)
...
def ave_brainregions(cohInputDict, brainRegions):
    ...
    for band in cohInputDict['Coherence'].keys():
        cohData_coherence[band] = average_coh_matrices(cohInputDict['Coherence'][band], brainRegions)
        cohData_phase[band] = average_coh_matrices(cohInputDict['Phase'][band], brainRegions)
...
def average_coh_matrices(cohData, brainRegions):
    ...
    for nn in range(nTimePoints):
        ...
        for pair in ijPairs:
            ...
            meanCoherence = np.mean(matrix[firstElectrodeA:lastElectrodeA, firstElectrodeB:lastElectrodeB])
        ...
        for diagonal in refNumber:
            ...
            meanCoherence = np.mean(matrix[firstElectrode:lastElectrode,firstElectrode:lastElectrode])
    ...
    if isinstance(cohData, dict):
        ...
        if calcPC == True:
            ...
            for band in data.keys():
                ...
                if sparsePCA == True:
                    pc[band], pcEigV[band] = sparse_pca_calculation(data[band], nCs)

                else:
                    pc[band], pcEigV[band] = pca_calculation(data[band], nCs)
                    
                    # Percent variance explained by each PC
                    plt.plot(pcEigV[band]); plt.title('Percent variance explained by PCA')
                    plt.ylabel('Fraction')
                    plt.xlabel('PC')
...
#            scipy.io.savemat(PCsavefile, {'PCA': pc})
            ...
            # Percent variance explained by each PC
            plt.plot(pcEigV); plt.title('Percent variance explained by PCA')
            plt.ylabel('Fraction')
            plt.xlabel('PC')
            plt.show()
...
def pca_calculation(dataMatrixIn, nPCs, meanSubtract=True, useHalfMatrix=True):
    ...
    pc = np.reshape(pca.fit_transform(X), (sz[0], sz[1], nPCs))  # Reconstruct signals
...
def ica_calculation(dataMatrixIn, nICs, useHalfMatrix=True, **kwargs):
    ...
    if kwargs and 'max_iter' in kwargs:
        ica = FastICA(n_components=nICs, max_iter=kwargs['max_iter'], tol=0.001)
    else:
    ...
    try:
        ic = np.reshape(ica.fit_transform(X), (sz[0], sz[1], nICs) )  # Reconstruct signals
...
def combine_coherence_files(input_dir, coh_str, fileExtension='.p', excludeGrid=False):
    ...
    for cohFile in cohFiles:
        ...
        print 'Loading file ' + str(kk+1) + ' out of ' + str(len(list(cohFiles)))
        ...
        if excludeGrid == False:
            ...
            for band in Coherence.keys():
                if kk == 0: # assign matrices on first case
                    ...
                    coh_allT[band] = np.concatenate((coh_allT[band], Coherence[band]), axis = -1)    
                    phase_allT[band] = np.concatenate((phase_allT[band], Phase[band]), axis = -1)
            ...
            for band in Coherence.keys():
                if kk == 0: # assign matrices on first case
                    ...
                    coh_allT[band] = np.concatenate((coh_allT[band], Coherence[band][64:,64:,:]), axis = -1)    
                    phase_allT[band] = np.concatenate((phase_allT[band], Phase[band][64:,64:,:]), axis = -1)            
...
def get_region_subset(patient, brainRegs, band, dataType='coherence', tFrame=10, excludeGrids=True, matType='full', **kwargs):
    ...
    if matType == 'full':
        ...
        # Get subset matrix
        regs, regsNew = electrodeinfo.choose_electrodes(patient.ID, brainRegs, no64Grid=no64Grid, excludeGrids=excludeGrids)
...
def find_ICs_from_PCA(dataMatrix, nPCs=50, meanSubtract=True, useHalfMatrix=True, method='ICA', **kwargs):
    ...
    print 'Running PCA'
    PCs, eigVs = pca_calculation(dataMatrix, nPCs=nPCs, useHalfMatrix=useHalfMatrix, meanSubtract=meanSubtract)
    ...
    if method.lower() == 'ica':
        print 'Running ICA'    
        ICs = ica_calculation(PCs[:,:,:nSig], nICs=nSig, useHalfMatrix=useHalfMatrix, **kwargs)
...
def find_ICNs(patient, brainRegs, band, tFrame=10, method='ICA', excludeGrid=True, matType='full', **kwargs):
    ...
    cohsSubset = get_region_subset(patient, brainRegs, band, dataType='coherence', tFrame=tFrame, excludeGrids=excludeGrid, matType=matType, **kwargs)

    # Find ICNs and projections
    icnMat = find_ICs_from_PCA(cohsSubset['coherence'], method=method, **kwargs)  
    icnProj = project_onto_component(cohsSubset['coherence'], icnMat, symmMat=True)
