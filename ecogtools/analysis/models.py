...
def extract_ICN_features(patient, proj, projtAxis, metric='IMS', deltaT=-300, interval=600, window=60, calcVar=True, plotCorr=False, shuffle=False):
    ...
    if metric == 'IMS':
        ...
        scoreTimes = np.array([time.mktime(tt.timetuple()) for tt in patient.IMS['IMStimes']])
    elif metric == 'pain':
        scorePts = np.array(patient.painScores['painScore'])
        scoreTimes = np.array([time.mktime(tt.timetuple()) for tt in patient.painScores['painTimes']])
    else:
    ...
    for ind in range(nICN):
        ...
        if calcVar == True:
            ...
            feature, featuretAxis = IMSmodule.calculate_coh_variance(proj[ind], projtAxis, window=window)
        ...
        scoreDataframe = IMSmodule.score_vs_feature(scorePts, scoreTimes, feature, featuretAxis, deltaT=deltaT, interval=interval)
        ...
        if plotCorr == True:
            ...
            slope, intercept, r, p, stderr = plotcorrelations.scatter_linregress(xFeature, yScore, ax=ax)
    ...
    if regType is not None:
        ...
        if crossVal == True:        
            if regType == 'Lasso':
                ...
                linModel = linear_model.ElasticNetCV(normalize=normalize, l1_ratio=l1_ratio_CV, cv=nIMS)
            else:
            ...
            if regType == 'Lasso':
                ...
                linModel = linear_model.ElasticNet(normalize=normalize, l1_ratio=l1_ratio, alpha=alpha)
            else:
                print('Unknown regularization type')        
    else:
