...
class Patient(object):
    ...
    def getBrainRegions(self):
        ...
        try:
            ...
            self.brainRegionsDict = electrodeinfo.electrode_locs(self.ID)['brainRegions']
            self.brainRegionsDict_abbr = electrodeinfo.electrode_locs(self.ID)['brainRegions_abbr']
        except:
    ...
    def getAnalysisFiles(self):
        ...
        try:     
            self.signalInfo = loaddata.signals_dataframe(self.ID, self.signalsPath)
        except:
    ...
    def loadSignals(self, tStamp=None, deltaIdx=0):
        ...
        sigout, fileID = loaddata.reload_signals_from_timestamp(self.signalInfo, timeStamp=tStamp, deltaIdx=deltaIdx)
    ...
    def loadCoherence(self, band, tBin, matType='average'):
        ...
        self.getAnalysisFiles()
        cohOut, fileID = loaddata.reload_coherence(self.cohInfo, band, tBin, matType=matType)
    ...
    def chooseElectrodes(self, elecAbbrs, no64Grid=False, excludeGrids=False):
        ...
        regs, regsNew = electrodeinfo.choose_electrodes(self.getID(), elecAbbrs, no64Grid=self.no64Grid, excludeGrids=excludeGrids)
    ...
    def subsetMatrix(self, elecAbbrs, band, tBin, dataType='coherence'):
        ...
        subsetInds = [kk for kk in itertools.chain.from_iterable(regs.values())]
...
class EcogSignals(object):
    ...
    def __init__(self, pID, dataPath=None):
        ...
        self.allSignals = loaddata.signals_dataframe(self.patient.getID(), self.patient.signalsPath)
    ...
    def loadSignals(self, tStamp=None, deltaIdx=0):
        ...
        sigout, fileID = self.patient.loadSignals(tStamp=tStamp, deltaIdx=deltaIdx)
    ...
    def getSubsetSignals(self, brainReg=None, excludeGrids=True, tStamp=None, nMin=5, deltaIdx=0, center=False):
        ...
        if self.signals is not None:
            self.subsetSignals, self.subsetTime = loaddata.get_subset_signals(self.signals, self.tAxis['UTC'], elecs, tStamp=tStamp, nMin=nMin, center=center)
        else:
    ...
    def getElecs(self, brainReg=None, excludeGrids=True):
        ...
        if brainReg == None:
            brainReg = raw_input("Enter region of interest. Choose from "+str(self.patient.brainRegionsDict.keys())+" or 'all': ")
        ...
        if len(brainReg) == 1:
            ...
            regs, regsNew = electrodeinfo.choose_electrodes(self.patient.ID, brainReg, excludeGrids=excludeGrids)
    ...
    def filterSignals(self, filtFqs):
        ...
        if not hasattr(self, 'subsetSignals'):
            ...
            self.signalsFilt = fltrs.butter_bandpass_filter(self.subsetSignals, self.sampFq, filtFqs[0], filtFqs[1])
    ...
    def sampleTimeAxis(self, nSamples=100):
        ...
        tStart = self.allSignals.iloc[0]['startT']+datetime.timedelta(minutes=60)
...
class SignalArray(object):
    ...
    def plotSignals(self, indices='all'):
        ...
        q = plotsignals.plot_data_scrollwheel(self.signals, self.tAxis, indices)
        
        return q
        
    def calculatePower(self, zscore=True, plotPower=False):
        
        self.f, self.Pxx = pwrmodule.powerspectrum(self.signals, self.sampFq, zscore=zscore)
